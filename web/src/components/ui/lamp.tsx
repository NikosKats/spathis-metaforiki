...
// Aceternity-style "Lamp" container, themed to the Spathis brand: deep ink
// background with a brand-red conic light beam instead of the original
// slate-950 + cyan combo. Drop content inside as children.
...
export const LampContainer = ({
  ...
  return (
    <div
      className={cn(
        'relative z-0 flex min-h-[680px] w-full flex-col items-center justify-center overflow-hidden bg-ink',
        className,
      )}
    >
      <div className="relative isolate z-0 flex w-full flex-1 scale-y-125 items-center justify-center">
        ...
        <div className="absolute top-1/2 h-48 w-full translate-y-12 scale-x-150 bg-ink blur-2xl"></div>
        ...
        <motion.div
          ...
          transition={{ delay: 0.3, duration: 0.8, ease: 'easeInOut' }}
          className="absolute inset-auto z-30 h-36 w-64 -translate-y-[6rem] rounded-full bg-[var(--brand)] blur-2xl"
        />
        <motion.div
          initial={{ width: '15rem' }}
          whileInView={{ width: '30rem' }}
          transition={{ delay: 0.3, duration: 0.8, ease: 'easeInOut' }}
          className="absolute inset-auto z-50 h-0.5 w-[30rem] -translate-y-[7rem] bg-[var(--brand)]"
        />
        <div className="absolute inset-auto z-40 h-44 w-full -translate-y-[12.5rem] bg-ink"></div>
      </div>

      <div className="relative z-50 flex -translate-y-24 flex-col items-center px-5">{children}</div>
    </div>
  );
};
